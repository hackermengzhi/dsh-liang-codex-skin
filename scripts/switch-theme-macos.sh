...
THEME_VIDEO="$("$NODE" -e '
const value = JSON.parse(process.argv[1]);
if (value.video !== null && typeof value.video !== "string") process.exit(1);
process.stdout.write(value.video || "");
' "$STAGE_RESULT")" || fail "Theme staging returned an invalid video identity."
...
[ -z "$THEME_VIDEO" ] || {
  ...
  [ "$VIDEO_BYTES" -gt 0 ] && [ "$VIDEO_BYTES" -le 10485760 ] \
    || fail "Theme intensity video must be non-empty and no larger than 10 MiB."
}
/bin/chmod 600 "$stage/"*
...
for entry in "$THEME_DIR"/*; do
  ...
  [ -n "$SAFE_CSS_NAME" ] && [ "$entry_name" = "$SAFE_CSS_NAME" ] && continue
