...
final class CoreTests: XCTestCase {
  ...
  func testSavedThemesCollapseOnlyBundledAliasAndKeepCurrentSelection() {
    let themes = [
      SavedThemeOption(id: "liang-ancestor", name: "滑动变祖 · 梁祖"),
      SavedThemeOption(id: "preset-liang-ancestor", name: "滑动变祖 · 梁祖"),
      SavedThemeOption(id: "forest", name: "Forest"),
      SavedThemeOption(id: "preset-forest", name: "Forest")
    ]

    XCTAssertEqual(
      Set(deduplicatedSavedThemes(themes, currentThemeID: "preset-liang-ancestor").map(\.id)),
      Set(["preset-liang-ancestor", "forest", "preset-forest"])
    )
    XCTAssertEqual(
      Set(deduplicatedSavedThemes(themes, currentThemeID: "liang-ancestor").map(\.id)),
      Set(["liang-ancestor", "forest", "preset-forest"])
    )
  }
