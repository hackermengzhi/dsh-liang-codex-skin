...
((cssText, artDataUrl, intensityVideoDataUrl, themeConfig) => {
  ...
  const intensityVideoUrl = (() => {
    if (!INTENSITY || typeof intensityVideoDataUrl !== "string" || !intensityVideoDataUrl) return "";
    const comma = intensityVideoDataUrl.indexOf(",");
    const mime = /^data:([^;,]+)/.exec(intensityVideoDataUrl)?.[1] || "video/webm";
    const binary = atob(intensityVideoDataUrl.slice(comma + 1));
    const bytes = new Uint8Array(binary.length);
    for (let index = 0; index < binary.length; index += 1) bytes[index] = binary.charCodeAt(index);
    return URL.createObjectURL(new Blob([bytes], { type: mime }));
  })();
  ...
  const applyIntensity = (rawLevel, { persist = false } = {}) => {
    ...
    if (root) {
      ...
      if (intensityVideoUrl && !intensityParts?.media.hasAttribute("data-video-error")) {
        setStyleProperty(root, "--dream-skin-art", "linear-gradient(90deg, #111 0%, #111 44%, #0b0a09 100%)");
      }
      const eased = ratio ** 1.65;
      const channel = (from, to) => Math.round(from + (to - from) * eased);
      const accent = { r: channel(120, 193), g: channel(124, 154), b: channel(118, 73) };
      const accentAlt = { r: channel(166, 213), g: channel(169, 181), b: channel(161, 110) };
  ...
  const ensureIntensity = (home) => {
    ...
    if (!media || !control || !intensityParts) {
      ...
      const onMetadata = () => { video.pause(); applyIntensity(intensityLevel); };
      const onError = () => {
      ...
      intensityParts = {
        control, input, media, output, video,
        dispose() {
  ...
  const ensure = ({ root: rootPass = true, scope: scopePass = false, parts: partPass = false } = {}) => {
    ...
    const scope = scopePass ? refreshScope() : window[STATE_KEY]?.scope;
    ensureIntensity(scope?.baseState === "home");
  };
  ...
  const cleanup = () => {
    ...
    root?.removeAttribute("data-liang-intensity");
    root?.removeAttribute("data-liang-stage");
    root?.style.removeProperty("--liang-intensity");
    root?.style.removeProperty("--liang-level");
  ...
  ensureIntensity(initialScope?.baseState === "home");
