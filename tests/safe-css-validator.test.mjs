...
const validatorPaths = existsSync(monorepoValidators[0])
  ? monorepoValidators
  : [path.join(repoRoot, "assets", "safe-css-validator.mjs")];
const policyPaths = existsSync(monorepoPolicies[0])
  ? monorepoPolicies
  : [path.join(repoRoot, "assets", "safe-css-policy.json")];
...
test("runtime and platform Safe CSS validators stay byte-identical", async () => {
  ...
  for (const file of files.slice(1)) assert.deepEqual(file, files[0]);
});
