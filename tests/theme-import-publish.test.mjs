...
const crossPlatformFixtureCandidate = path.join(
  projectRoot,
  "docs",
  "images",
  "gallery",
...
const crossPlatformFixtureImage = await fs.access(crossPlatformFixtureCandidate)
  .then(
    () => crossPlatformFixtureCandidate,
    () => path.join(here, "fixtures", "cross-platform-skin-01.jpg"),
  );
...
async function publishAndKill(stage, destinationRoot, phase) {
  const escapedKillHook = killHook.replaceAll("\\", "\\\\").replaceAll('"', '\\"');
  const nodeOptions = [process.env.NODE_OPTIONS, `--require="${escapedKillHook}"`]
    .filter(Boolean)
    .join(" ");
