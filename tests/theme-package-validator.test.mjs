...
const windowsInjectorCandidate = path.join(projectRoot, "windows", "scripts", "injector.mjs");
const hasWindowsRuntime = await fs.access(windowsInjectorCandidate).then(() => true, () => false);
const windowsInjector = hasWindowsRuntime ? windowsInjectorCandidate : macosInjector;
const { loadTheme: loadWindowsTheme } = hasWindowsRuntime
  ? await import("../../windows/scripts/injector.mjs")
  : { loadTheme: loadMacTheme };
