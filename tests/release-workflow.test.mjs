...
const standaloneWorkflow = path.resolve(here, "../.github/workflows/release.yml");
const monorepoWorkflow = path.resolve(here, "../../.github/workflows/release.yml");
const workflowPath = await fs.access(standaloneWorkflow)
  .then(() => standaloneWorkflow, () => monorepoWorkflow);
if (!await fs.access(workflowPath).then(() => true, () => false)) {
  console.log("SKIP: standalone source archive does not publish through the monorepo workflow.");
  process.exit(0);
}
